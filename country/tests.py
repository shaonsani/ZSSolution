...
class CountryListTestCase(TestCase):
    ...
    def test_country_list(self):
        # get API response
        response = client.get(f'{self.country_list_url}?page=1&limit=10', HTTP_AUTHORIZATION='Token ' + self.token.key)
        # get data from db
    ...
    def test_country_list_error_response(self):
        # get API response
        response = client.get(f'{self.country_list_url}', HTTP_AUTHORIZATION='Token ' + self.token.key)
...
class StateListTestCase(TestCase):
    ...
    def test_state_list(self):
        # get API response
        response = client.get(f'{self.state_list_url}?page=1&limit=10', HTTP_AUTHORIZATION='Token ' + self.token.key)
        # get data from db
    ...
    def test_state_list_error_response(self):
        # get API response
        response = client.get(f'{self.state_list_url}', HTTP_AUTHORIZATION='Token ' + self.token.key)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class AddressListTestCase(TestCase):
    ...
    def test_address_list(self):
        # get API response
        response = client.get(f'{self.address_list_url}?page=1&limit=10', HTTP_AUTHORIZATION='Token ' + self.token.key)
        # get data from db
    ...
    def test_address_list_error_response(self):
        # get API response
        response = client.get(f'{self.address_list_url}', HTTP_AUTHORIZATION='Token ' + self.token.key)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class AddressDetailTestCase(TestCase):
    ...
    def setUp(self):
        self.india = Country.objects.create(name='India', latitude=37.45, longitude=14.45, code='IND')
        self.mumbai = State.objects.create(name='Mumbai', country=self.india)
        self.address = Address.objects.create(name='baitul aman housing', house_number='580', road_number=12,
                                              state=self.mumbai)
        self.user = User.objects.create_superuser(username='sani', email='sani@gmail.com', password='sani1234')
        self.token = Token.objects.create(user=self.user)
